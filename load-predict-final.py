...
X_scaler = joblib.load('pickle-final/X_scaler.pkl') 
y_scaler = joblib.load('pickle-final/y_scaler.pkl') 
imp = joblib.load('pickle-final/Imputer.pkl') 
vec = joblib.load('pickle-final/Vector.pkl') 
...
for estimator in estimators:
	...
	#print y_test[1]
 	...
 	model = joblib.load('pickle-final/' + estimator['name'] + '.pkl') 
	...
	np.savetxt('pickle-final/' + estimator['name'] + '-result.csv', result, delimiter=",", fmt="%s")

scores = np.asarray(validation_scores_all)
names = np.asarray(['KNN', 'GBR', 'RF', 'SVR'])
score = np.column_stack((names, scores))
np.savetxt("pickle-final/validScores-newData.csv",	score, delimiter=",", fmt="%s")
